...
총 글자 수: 2000-3000자 (공백 포함) 분량의 상세하고 유용한 콘텐츠

중요 지침:
- 핵심 키워드 "${primaryKeyword}"를 전체 글에서 15-20회 자연스럽게 사용
- 각 단락마다 최소 1-2회 핵심 키워드 포함
- 보조 키워드도 각각 3-5회씩 분산 배치
- 키워드 밀도 목표: 2-3%
`;
...
구조:
1. 도입부(400-500자): 
   - 강력한 첫 문장에 "${primaryKeyword}" 포함
   - 이 글을 읽으면 얻을 수 있는 구체적 가치 제시
   - 글의 구성 미리보기
   - 핵심 키워드 3-4회 자연스럽게 포함

2. 본문 3개 섹션(각 500-600자):
   섹션1 - 기초/원인/이유 (WHY): 
   - ${subKeywords[0] || '기본 개념'} 중심 설명
   - "${primaryKeyword}"를 각 단락에 2-3회 포함
   - 구체적 데이터와 연구 결과 인용
   
   섹션2 - 구체적 방법 1 (HOW):
   - ${subKeywords[1] || '실용적 방법'} 중심 설명  
   - "${primaryKeyword}"와 연계한 실행 방법
   - 각 단락에 핵심 키워드 2-3회 포함
   
   섹션3 - 구체적 방법 2 (HOW):
   - ${subKeywords[2] || '고급 팁'} 중심 설명
   - "${primaryKeyword}" 활용 주의사항
   - 각 단락에 핵심 키워드 2-3회 포함

3. 결론(300-400자):
   - "${primaryKeyword}" 핵심 내용 요약
   - 오늘 당장 실천할 수 있는 액션 제시
   - 핵심 키워드 2-3회 포함
...
구조 (PAS 프레임워크 적용):
1. 문제 공감(400-500자):
   - "${primaryKeyword}"로 고통받는 상황 묘사
   - "이게 내 얘기네" 반응 유도
   - 핵심 키워드 3-4회 포함
   - ${subKeywords[0] || '고객 고통점'} 키워드 활용

2. 문제 심화 및 해결책 제시(1000-1200자):
   섹션1 - 문제 심화(400자):
   - "${primaryKeyword}" 문제 해결 안 했을 때의 위험
   - ${subKeywords[1] || '문제점'} 구체적 사례
   - 핵심 키워드 3-4회 포함
   
   섹션2 - 해결책 등장(400자):
   - "${primaryKeyword}" 완벽 해결책 소개
   - ${subKeywords[2] || '해결책'} 중심 베네핏
   - 핵심 키워드 3-4회 포함
   
   섹션3 - 증거 제시(400자):
   - "${primaryKeyword}" 성공 사례
   - Before/After 극적 비교
   - 핵심 키워드 2-3회 포함

3. 행동 유도(400-500자):
   - "${primaryKeyword}" 특별 오퍼
   - 희소성/긴급성 부여
   - 명확한 CTA
   - 핵심 키워드 2-3회 포함
